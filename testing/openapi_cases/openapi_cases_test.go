...
func PositiveCasesTest(validator *validation.ValuesValidator, moduleName string, testCases *TestCases) error {
	for _, testCase := range testCases.Positive.ConfigValues {
		err := ValidateCase(validator, moduleName, validation.ConfigValuesSchema, testCase, testCases.hasFocused)
		if err != nil {
			return err
		}
	}
	for _, testCase := range testCases.Positive.Values {
		err := ValidateCase(validator, moduleName, validation.ValuesSchema, testCase, testCases.hasFocused)
		if err != nil {
			return err
		}
	}
	for _, testCase := range testCases.Positive.HelmValues {
		err := ValidateCase(validator, moduleName, validation.HelmValuesSchema, testCase, testCases.hasFocused)
		if err != nil {
			return err
		}
	}
...
func NegativeCasesTest(validator *validation.ValuesValidator, moduleName string, testCases *TestCases) error {
	for _, testCase := range testCases.Negative.ConfigValues {
		err := ValidateCase(validator, moduleName, validation.ConfigValuesSchema, testCase, testCases.hasFocused)
		if err == nil {
			return fmt.Errorf("negative case error for config values: must be invalid: %+v", ValuesToString(testCase))
		}
	}
	for _, testCase := range testCases.Negative.Values {
		err := ValidateCase(validator, moduleName, validation.ValuesSchema, testCase, testCases.hasFocused)
		if err == nil {
			return fmt.Errorf("negative case error for values: must be invalid: %+v", ValuesToString(testCase))
		}
	}
	for _, testCase := range testCases.Negative.HelmValues {
		err := ValidateCase(validator, moduleName, validation.HelmValuesSchema, testCase, testCases.hasFocused)
		if err == nil {
			return fmt.Errorf("negative case error for helm values: must be invalid: %+v", ValuesToString(testCase))
		}
	}
	return nil
}
