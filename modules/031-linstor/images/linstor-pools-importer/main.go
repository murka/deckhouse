...
func parseInputs() (options, error) {
	var opts options
	opts.delaySeconds = *flag.Int("delay", 10, "Delay in seconds between scanning attempts")
	klog.Infof("delaySeconds: %d", opts.delaySeconds)
...
func provisionStoragePools(ctx context.Context, lc *lclient.Client, kc kclient.Client, owner *v1.ObjectReference, nodeName string, delay time.Duration) error {
	...
	for {
		...
		case cand := <-candiCh:
			...
			if err != nil {
				...
				logAndCreateFailEvent(ctx, kc, nodeName, owner, "Failed to sync LINSTOR storage pool: "+err.Error()) // + err handling
				return err
			}

			if changed {
				if err := logAndCreateCreationEvent(ctx, kc, nodeName, owner, "Created LINSTOR storage pool: "+nodeName+"/"+cand.StoragePool.StoragePoolName); err != nil {
					return err
				}
			} else {
			...
			if err != nil {
				// only abortions possible here
				logAndCreateFailEvent(ctx, kc, nodeName, owner, "Failed to generate Kubernetes storage classes: "+err.Error()) // + err handling
				return err
			}
			...
			for _, sc := range scs {
				...
				if err != nil {
					// only abortions possible here
					logAndCreateFailEvent(ctx, kc, nodeName, owner, "Failed to sync Kubernetes storage class: "+err.Error()) // + err handling
					return err
				}

				if changed {
					if err := logAndCreateCreationEvent(ctx, kc, nodeName, owner, "Created Kubernetes storage class: "+sc.GetName()); err != nil {
						return err
					}
				} else {
					klog.Info("Kubernetes storage calss " + sc.GetName() + " is already configured")
				}
			}
...
func main() {
	...
	klog.Infof("Starting main loop (delay: %d seconds)", opts.delaySeconds)

	// Main loop: configure storage pools and create storage classes
	stop := make(chan struct{})
	go func() {
		defer cancel()
		err := provisionStoragePools(ctx, lc, kc, &owner, opts.nodeName, time.Duration(opts.delaySeconds)*time.Second)
...
// Sends creation event to Kubernetes
func logAndCreateCreationEvent(ctx context.Context, kc kclient.Client, nodeName string, owner *v1.ObjectReference, message string) error {
	klog.Info(message)
	event := newKubernetesEvent(nodeName, owner, v1.EventTypeWarning, "Failed", message)
	return kc.Create(ctx, &event)
}

// Sends failed event to Kubernetes, abort if failed
func logAndCreateFailEvent(ctx context.Context, kc kclient.Client, nodeName string, owner *v1.ObjectReference, message string) {
	klog.Info(message)
...
func parseNameFromLVMTags(tags *[]string) (string, error) {
	...
	for _, tag := range *tags {
		...
		if t[0] == linstorPrefix && t[1] != "" {
			foundNames = append(foundNames, t[1])
		}
	}
